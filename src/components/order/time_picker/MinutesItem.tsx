...
function MinutesItem({ item, hour, setMinutes }: IMinutesItem) {
    ...
    const minutesClickHandler = (item: ExtendedIMinutes,e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
        if (item.isActive) {
            store.setTime('')
            setMinutes((t) => {
                return [...t].map((time) => {
    ...
    return (
       <div onClick={(e)=>{minutesClickHandler(item,e)}} className={`minutesButton px-1 ${item.isActive?"bg-blue-500 text-white ":"bg-red-500"}`}>
         {item.value}
        </div> 
    );
}
